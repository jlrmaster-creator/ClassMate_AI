        ...
        {projects.map((project) => {
          ...
          return (
            <article className="project-card" key={project.id}>
              <div className="project-card-top"><span className="project-icon"><FolderKanban size={20} /></span><div className="project-actions">{project.code && <span className="share-wrap"><button className="share-code-button" onClick={() => copyCode(project.code)} title="Compartir por código" aria-label={`Compartir ${project.title} por código ${project.code}`}>{copiedCode === project.code ? <Check size={15} /> : <Share2 size={15} />}<span className="share-code-text">{project.code}</span></button>{copiedCode === project.code && <span className="code-copied-toast">¡Copiado!</span>}</span>}<button onClick={() => openEditProject(project)} aria-label={`Editar ${project.title}`}><Edit3 size={16} /></button>{isOwner && <button onClick={() => onDelete(project)} aria-label={`Eliminar ${project.title}`}><Trash2 size={16} /></button>}</div></div>
              <h3>{project.title}</h3>
